...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Backend returns a URL like http://localhost:5173/reset-password?token=xxx
      // Extract the token and rebuild with the current domain so it works on any deployment
      const backendUrl = res.data.data;
      const token = new URL(backendUrl).searchParams.get('token');
      const correctUrl = `${window.location.origin}/reset-password?token=${token}`;
      setResetUrl(correctUrl);
